...
try {
	...
	echo "<br>";
	$maincategory = "code-scripts";
	$subcategory = "free-code-scripts"; 
	...
	for ($i=2; $i < 16; $i++) { 
		# code...
		$url = 'http://www.jqueryscript.net/jquery-plugins/list-16-'.$i.'.html';
		// Retrieve the DOM from a given URL
		$html = file_get_html($url,false);
		foreach($html->find('.jqoverlay article') as $e) {
			// small image
			$small_image = $e->getElementByTagName('img')->src;
			// title
			$title = $e->getElementByTagName('h2')->getElementByTagName('a')->innertext;
			// slug url
			...
			$result = $stmt->setFetchMode(PDO::FETCH_ASSOC); 
		   	...
		   	if ($item == null) {
		   		// link
				$link = $e->getElementByTagName('h2')->getElementByTagName('a')->href;
				$html = file_get_html($link,false);
				// big image
				$big_image = $html->find('.vlightbox2 img',0)->src;
				// description
				// htmlentities(htmlspecialchars(thehmldata));
				$description = htmlentities(htmlspecialchars($html->find('.entry p',0)->innertext));
				
				$meta_description = strip_tags($html->find('.entry p',0)->innertext);
				
				
				$stmt = $conn->prepare("INSERT INTO item (title,link,small_picture_link,big_picture_link,description,category_id,meta_element,meta_description,slug,created_at,updated_at) values (?,?,?,?,?,?,?,?,?,?,?)");

				$stmt->execute([$title,$link,$small_image,$big_image,$description,$category['id'],$meta_element,$meta_description,$slug,date('Y-m-d H:i:s'),date('Y-m-d H:i:s')]);
