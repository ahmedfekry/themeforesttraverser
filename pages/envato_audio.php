...
function fetch_date_2($mainCategory,$category_name,$main_link,$number_of_pages,$sub_link)
{
	$response = array();
	try {
		...
		for ($i=1; $i <= $number_of_pages; $i++) { 
			...
			foreach($html->find('.tsunami-item') as $e) {
				// small image
				$small_image = $e->find('.item-thumbnail__image img',0)->src;
				echo $small_image;
				die();
				// title
