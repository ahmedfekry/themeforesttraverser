...
try {
	...
	echo "<br>";
		$mainCategory = "video-effectss";
		$website_link = ""; // could be null
		$category_name = "after-effects-project-filess"; 
	...
	for ($i=1; $i < 3; $i++) { 
		# code...
		// https://videohive.net/category/after-effects-project-files?page=2&referrer=search&utf8=%E2%9C%93&view=list
		$url = 'https://videohive.net/category/after-effects-project-files?page='.$i.'&referrer=search&utf8=%E2%9C%93&view=list';
		...
		foreach($html->find('.product-list__columns-container') as $e) {
			...
			$result = $stmt->setFetchMode(PDO::FETCH_ASSOC); 
		   	...
		   	if ($item == null) {
		   		// link
				$link = "https://videohive.net".$e->getElementByTagName('h3')->getElementByTagName('a')->href;
				$html = file_get_html($link,false);
				// big image
				$big_image = $html->find('.item-preview a img',0)->src;
				// video
				$video_link = $html->find('.video-preview-wrapper meta',2)->content;
				// echo $video_link;
				// die;
				// meta-attributes 
				...
				$stmt = $conn->prepare("INSERT INTO item (title,link,small_picture_link,big_picture_link,price,description,meta_data,category_id,meta_element,meta_description,slug,created_at,updated_at,video_link) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?)");

				$stmt->execute([$title,$link,$small_image,$big_image,$price,$description,$meta_attributes,$category['id'],$meta_element,$meta_description,$slug,date('Y-m-d H:i:s'),date('Y-m-d H:i:s'),$video_link]);
